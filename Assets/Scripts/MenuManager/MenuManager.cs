...
public class MenuManager : MonoBehaviour
{
    ...
    public static event Action<MenuCanvas> menuChanged;
    ...
    private readonly Dictionary<AssetEnum, int> _menuCameraPosOverrides = new()
    {
    ...
    {
        int targetCameraPosIndex = _mainMenusStack.Count;

        bool containsIndexOverride = _menuCameraPosOverrides.ContainsKey(assetEnum);
        if (containsIndexOverride)
        {
            targetCameraPosIndex = _menuCameraPosOverrides[assetEnum];
        }
        ...
        if (isCurrentMenuRequested && _mainMenusStack.Any())
        {
            if (!containsIndexOverride)
            {
                targetCameraPosIndex = _mainMenusStack.Count - 1;
            }
    ...
    private void OnChangeMenuComplete(AssetEnum assetEnum)
    {
        ...
        menuChanged?.Invoke(_currentMainMenu);
    }
    ...
    public void OnBackPressed()
    {
        ...
        if (_menuCameraPosOverrides.ContainsKey(_currentMainMenu.GetAssetEnum()))
        {
            targetIndex = _menuCameraPosOverrides[_currentMainMenu.GetAssetEnum()];
        }
